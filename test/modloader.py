...
def download_fabric(version:str,name:str,progress_var,on_complete):
...
def download_server_jar(name:str, version:str,progress_var,on_complete,modloader:str):
    ...
    if(modloader not in valid_mod_loaders):
        return -2
    if(modloader == "fabric"):
        download_fabric(name=name, version=version,progress_var=progress_var,on_complete=on_complete)
        return 0
